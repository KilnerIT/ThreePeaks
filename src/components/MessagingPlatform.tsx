...
export default function MessagingPlatform({
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl p-4 border-t-4 border-orange-400 border-x border-b border-slate-100 flex flex-col md:flex-row gap-4 h-auto md:h-[350px]">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-y-auto max-h-[200px] md:max-h-none pr-1 space-y-2.5 custom-scrollbar">
                ...
                >
                  ...
                  {post.image && (
                    <div className="mt-1.5 overflow-hidden rounded-lg border border-slate-150 shadow-inner max-h-[120px] bg-slate-50 relative flex items-center justify-center">
                      <img src={post.image} alt="Walk Snapshot" className="w-full object-cover max-h-[120px]" />
                    </div>
                  )}
                </motion.div>
              );
      ...
      <div className="w-full md:w-[220px] bg-slate-50 border border-slate-200 rounded-2xl p-2.5 flex flex-col justify-between shrink-0">
        <div className="space-y-1.5">
          <div className="grid grid-cols-2 gap-1.5">
            <div>
              <label className="block text-[7px] font-black text-slate-400 uppercase tracking-wider">
                Author
              </label>
              <select
                value={newAuthor}
                onChange={(e) => setNewAuthor(e.target.value)}
                className="w-full text-[10px] font-bold text-slate-700 bg-white border border-slate-300 rounded p-1 focus:outline-none"
              >
          ...
          <div>
            <textarea
              ...
              placeholder="Update (e.g. 'Ascending Peak!')"
              className="w-full text-[10px] font-medium text-slate-800 bg-white border border-slate-200 rounded p-1.5 focus:border-orange-400 focus:outline-none placeholder:text-slate-400"
            />
          </div>
          ...
          <div
            ...
            className={`border border-dashed rounded-lg p-1.5 text-center cursor-pointer transition-all ${
              dragActive ? 'border-orange-500 bg-orange-50' : 'border-slate-250 bg-white hover:bg-slate-100'
            }`}
          >
            <input ref={fileInputRef} type="file" accept="image/*" onChange={handleFileInputChange} className="hidden" />
            {newImage ? (
              <span className="text-[8px] text-emerald-800 font-bold truncate block">📷 Image Attached!</span>
            ) : (
              <span className="text-[7.5px] font-black text-slate-400 uppercase block">📁 Click / drop photo</span>
            )}
          </div>
        </div>
        ...
        <button
          ...
          type="button"
          className="w-full bg-[#f59e0b] hover:bg-[#d97706] text-white font-black text-[9px] py-1.5 rounded uppercase tracking-wider shadow-xs cursor-pointer text-center mt-1.5"
        >
          Broadcast Status
        </button>
      </div>
